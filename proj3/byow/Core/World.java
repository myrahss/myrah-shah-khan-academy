...
public class World {
    ...
    public void genWorld() {
        ...
        Collections.sort(roomList, new Comparator<Room>() {
            ...
            public int compare(Room r1, Room r2) {
                return Integer.compare(r2.getC().getY(), r1.getC().getY());
            }
        });
    ...
    private void drawPlayer() {
        worldTiles[player.getP().getX()][player.getP().getY()] = Tileset.AVATAR;
    }
    private void drawTarget() {
        worldTiles[target.getP().getX()][target.getP().getY()] = Tileset.LOCKED_DOOR;
    }
    ...
    private void drawHallwayConnectRooms() {
        for (int i = 0; i < roomList.size() - 1; i++) {
            ...
            Room.drawHallway(worldTiles, r1.getC(), r2.getC());
        }
    }
    ...
    public boolean overlap(Room room) {
        for (int i = 0; i < room.getWidth(); i++) {
            for (int j = 0; j < room.getHeight(); j++) {
                if (worldTiles[room.getP().getX() + i][room.getP().getY() + j] != Tileset.GRASS) {
                    return true;
                }
            }
        }
